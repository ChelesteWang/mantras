...
Mantras 项目基于现代 AI 代理的核心理念：**AI 代理 = 人格 + 提示 + 工具**
...
所有组件都被视为统一的资产，通过 `UnifiedAssetManager` 进行集中管理：
...
### 0. 统一资产管理器 (Unified Asset Manager)
...
### 0.2. 资产加载器 (Asset Loader & Serializer)
- **文件**: `asset-loader.ts`
- **功能**: 资产的导入导出和序列化
- **特性**:
  - 从文件、目录、URL加载资产
  - 资产验证和过滤
  - 多种格式的导出（JSON、TypeScript定义）
  - 按类型分组保存
