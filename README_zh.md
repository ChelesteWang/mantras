...
欢迎来到 Mantras！✨ Mantras 是一个基于 TypeScript 的创新框架，旨在赋能 AI 智能体在集成开发环境（IDE）中无缝运行。我们的核心理念是让智能体能够轻松学习、管理和应用各种“咒语”（Mantras）——我们称之为“Item”或“Tool”的强大技能或工具，所有这些都构建在灵活的 `IMantra` 基础接口之上。我们坚信模块化的力量，Mantras 允许智能体逻辑与其使用的工具之间实现清晰的分离。智能体可以动态配置和管理，巧妙地利用 IDE 特有的上下文，并遵守预定义的规则，以优雅和精确地执行复杂且具有上下文感知和规则指导的任务。

## 🌟 特性

- **🧠 智能体学习与适应**：智能体可以动态学习和忘记“咒语”（工具/技能），从而实现持续进化和适应性。
- **🧩 模块化设计**：清晰分离智能体逻辑和工具，增强可维护性和可扩展性。
- **🔧 可配置工具**：通过 `ItemConfig` 定义工具，允许轻松定制和扩展。
- **⚙️ 动态智能体管理**：`AgentManager` 负责加载、实例化和管理智能体及其生命周期。
- **📚 中央注册表**：所有可用工具的统一注册表，便于发现和重用。
- **💡 IDE 上下文感知**：利用 `IDEContext`（如打开的文件、选定的文本）使智能体能够执行上下文相关的操作。
- **📜 规则驱动执行**：通过 `IRule` 定义智能体行为的约束和指南。
- **🚀 快速上手**：提供清晰的安装、构建和运行演示的说明。
- **🌐 多语言支持**：提供中英文文档。
- **🛠️ 示例工具**：包含如 `CodeFormatterTool` 之类的示例工具，以加速开发。
...
准备好开始使用 Mantras 了吗？请按照以下简单步骤操作：

1.  **克隆仓库**：
    ```bash
    git clone https://github.com/your-username/mantras.git
    cd mantras
    ```
...
我们为您准备了详细的文档，以帮助您更深入地了解 Mantras：

- **📖 [智能体培养指南 (中文)](./docs/agent-cultivation-guide_zh.md)**：学习如何“培养”您的 Mantras 智能体，掌握内部技能、实践咒语，并利用外部辅助工具。
- **🌌 [智能体进化指南 (英文)](./docs/agent-evolution-guide.md)**：探索 Mantras 智能体如何实现自我学习和优化，实现持续进化。
