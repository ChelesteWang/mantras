  ...
  return [
    {
      ...
      handler: async ({ userInput, context, analysisDepth }) => {
        const analysis = performIntentAnalysis(userInput, context || '', analysisDepth);
...
function analyzeEmotionalTone(input: string): string {
  ...
  if (input.includes('困惑') || input.includes('不懂') || input.includes('confused')) return 'confused';
  if (input.includes('问题') || input.includes('错误') || input.includes('problem')) return 'concerned';
  if (input.includes('好') || input.includes('棒') || input.includes('great')) return 'positive';
  return 'neutral';
}

function assessComplexity(input: string): string {
  const complexIndicators = ['架构', '系统', '复杂', '多个', '整体', 'architecture', 'system', 'complex'];
...
function extractKeywords(input: string): string[] {
  ...
  const stopWords = ['的', '是', '在', '有', '和', '了', '我', '你', 'the', 'is', 'in', 'and', 'to'];
...
function assessUrgency(input: string): string {
  if (input.includes('急') || input.includes('紧急') || input.includes('urgent') || input.includes('asap')) return 'high';
...
function calculateMatchScore(persona: any, analysis: any, requirements: string[]) {
  let capabilityScore = 0;
  let domainScore = 0;
  let styleScore = 0.5; // 基础分数

  // 能力匹配
  if (analysis.primaryIntent === 'technical' && persona.capabilities.technical) capabilityScore = 0.9;
  else if (analysis.primaryIntent === 'creative' && persona.capabilities.creative) capabilityScore = 0.9;
  else if (analysis.primaryIntent === 'analytical' && persona.capabilities.analysis) capabilityScore = 0.9;
  else if (analysis.primaryIntent === 'supportive' && persona.capabilities.empathetic) capabilityScore = 0.9;
  ...
  return {
    ...
    recommendation: overall > 0.7 ? 'highly_recommended' : overall > 0.5 ? 'recommended' : 'consider_alternatives'
  };
}
