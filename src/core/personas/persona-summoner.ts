...
export class PersonaSummoner {
    ...
    const personas: Persona[] = [
      {
        ...
        systemPrompt:
          'You are a professional data analyst. Always provide clear, data-driven insights with actionable recommendations. Use charts and metrics where relevant.',
        personality: {
      ...
      },
      {
        ...
        systemPrompt:
          'You are a creative writer. Craft engaging, imaginative content while maintaining clarity and readability. Use vivid descriptions and compelling narratives.',
        personality: {
      ...
      },
      {
        ...
        systemPrompt:
          'You are a senior technical expert. Provide detailed, accurate technical explanations. Break down complex concepts and provide code examples when relevant.',
        personality: {
      ...
      },
      {
        ...
        systemPrompt:
          'You are an empathetic listener. Provide supportive, understanding responses with genuine care. Focus on emotional well-being and practical advice.',
        personality: {
  ...
  public summonPersona(request: SummonRequest): SummonedPersona {
    ...
    sessionMemory.addMemory(
      ...
      8,
      ['session', 'persona', 'initialization']
    );
  ...
  private resolvePersonaByIntent(intent: string): Persona | undefined {
    ...
    if (
      intentLower.includes('tech') ||
      intentLower.includes('programming') ||
      intentLower.includes('code')
    ) {
      return this.personas.get('tech-expert');
    }
    if (
      intentLower.includes('support') ||
      intentLower.includes('help') ||
      intentLower.includes('understand')
    ) {
      return this.personas.get('therapist');
    }
    if (
      intentLower.includes('批评') ||
      intentLower.includes('审视') ||
      intentLower.includes('问题') ||
      intentLower.includes('挑战')
    ) {
      return this.personas.get('grumpy_bro');
    }
    if (
      intentLower.includes('深度') ||
      intentLower.includes('逻辑') ||
      intentLower.includes('完整') ||
      intentLower.includes('原理')
    ) {
      return this.personas.get('reflection_sis');
    }
    if (
      intentLower.includes('亮点') ||
      intentLower.includes('优势') ||
      intentLower.includes('创意') ||
      intentLower.includes('鼓励')
    ) {
      return this.personas.get('fan_girl');
    }
    if (
      intentLower.includes('商业') ||
      intentLower.includes('产品') ||
      intentLower.includes('用户') ||
      intentLower.includes('价值')
    ) {
  ...
  // 新增：为会话添加对话记录
  public addConversationToSession(
    sessionId: string,
