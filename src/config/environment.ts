...
export class ConfigManager {
  ...
  private loadFromEnvironment(): Partial<Config> {
    ...
    if (env.MONITORING_ENABLED !== undefined) config.monitoring.enabled = env.MONITORING_ENABLED !== 'false';
    if (env.METRICS_ENABLED !== undefined) config.monitoring.metricsEnabled = env.METRICS_ENABLED !== 'false';
    
    if (env.RATE_LIMIT_ENABLED !== undefined) config.security.rateLimiting.enabled = env.RATE_LIMIT_ENABLED !== 'false';
    if (env.RATE_LIMIT_MAX) config.security.rateLimiting.maxRequests = parseInt(env.RATE_LIMIT_MAX, 10);
